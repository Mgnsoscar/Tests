...
a lab supply (5 V, 50 mA limit) through a series resistor into the antenna
body, through the contact under test, down the antenna's
...
contact closed both channels read 2.35 V; when the antenna's own wiring
opens channel 1 drops to 0 V within nanoseconds and channel 2 rises to the
full 5 V (the supply is there, nothing draws current); when the supply
...
through 1.2 V in NORMAL mode with fast segmentation, so every opening and
...
dropout. The record analysis uses a hysteresis (open below 0.9 V, closed
again only above 1.5 V by default), so a signal lingering at the threshold
